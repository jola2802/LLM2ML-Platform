...
export function setupAPIEndpoints(app, upload, scriptDir, venvDir) {
  ...
  app.post('/api/analysis-cache/clear', async (req, res) => {
    try {
      ...
      const response = await fetch(`${process.env.MAS_SERVICE_URL || 'http://localhost:3002'}/api/data/cache/clear`, {
        method: 'POST',
  ...
  app.get('/api/analysis-cache/status', async (req, res) => {
    try {
      ...
      // Cache wird über Python-Service verwaltet
      const response = await fetch(`${process.env.MAS_SERVICE_URL || 'http://localhost:3002'}/api/data/cache/status`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });
