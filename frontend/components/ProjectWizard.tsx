      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-900/50 p-6 rounded-lg border border-slate-700">
          ...
          <p className="text-sm text-slate-400 mb-4">
            Describe here specific requirements, e.g. desired target variable, preferred features, preferred model type/algorithm, metrics or other constraints.
          </p>
          <textarea
            value={userPreferences}
            onChange={(e) => setUserPreferences(e.target.value)}
            placeholder="Example: I need a model that predicts the target variable churn."
        ...
        {isProcessingData && (
          <div className="bg-blue-900/30 border border-blue-500/50 rounded-lg p-6">
            <div className="flex items-center space-x-4">
              <Spinner size="sm" />
              <div>
                <h4 className="text-blue-400 font-medium">🧠 .. analyzing the manipulated data...</h4>
                <p className="text-blue-300 text-sm">Create optimized recommendations for your selection</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-gray-900/50 p-6 rounded-lg border border-gray-700">
          ...
          {llmRecommendations?.modelType && (
            <div className="p-3 bg-green-900/20 border border-green-500/30 rounded">
              <p className="text-green-300 text-sm">
                💡 <strong>AI-Recommendation:</strong> {llmRecommendations.modelType}
  ...
  return (
    <div className="max-w-4xl mx-auto bg-slate-800 rounded-lg shadow-xl p-6 sm:p-8 animate-fade-in">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-3xl font-bold text-white">Intelligent ML Wizard</h2>
          <p className="text-slate-400">Step {step} of {steps.length}: {steps[step - 1].title}</p>
        </div>
      ...
      <div className="flex items-center justify-between mb-6">
        {steps.map((stepInfo, index) => (
          <div key={index} className="flex items-center">
            ...
            <span className={`ml-2 text-sm ${index + 1 <= step ? 'text-white' : 'text-slate-400'
              }`}>
              {stepInfo.title}
            </span>
            {index < steps.length - 1 && (
              <div className={`mx-4 h-0.5 w-16 ${index + 1 < step ? 'bg-emerald-600' : 'bg-slate-600'
                }`} />
            )}
          </div>
        ))}
