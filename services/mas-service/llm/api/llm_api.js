...
export async function getLLMRecommendations(analysis, filePath = null, selectedFeatures = null, excludedFeatures = null, userPreferences = null) {
  let dataOverview = '';

  if (filePath) {
    try {
      const dataAnalysis = await pythonClient.analyzeData(filePath, false);
  ...
  if (!dataOverview) {
    ...
    dataOverview = `DATA INFORMATION:
- Available columns: ${filteredColumns.join(', ')}
- Number of rows: ${analysis.rowCount}
- Data types: ${Object.entries(filteredDataTypes).map(([col, type]) => `${col}: ${type}`).join(', ')}

EXAMPLE DATA (first 10 rows):
${filteredSampleData.map((row, i) =>
      `Row ${i + 1}: ${filteredColumns.map((col, j) => `${col}=${row[j]}`).join(', ')}`
    ).join('\n')}`;
  }

  const prompt = formatPrompt(LLM_RECOMMENDATIONS_PROMPT, {
    dataOverview,
    userPreferences: userPreferences ? userPreferences : 'No specific preferences provided.'
  });
  ...
  logAgentCall(WORKER_AGENTS.DATA_ANALYZER.key, agentModel, 'LLM recommendations for ML pipeline');
  console.log(`🤖 ${agentConfig.name} starts with model: ${agentModel}`);
  ...
  try {
    ...
    console.log('LLM recommendations successfully extracted:', {
    ...
    console.error('Error in LLM recommendations:', error);

    // Versuche es noch einmal mit Retry
    try {
      const response = await callLLMAPI(prompt, null, agentModel, agentConfig.retries || 3);
      const recommendations = extractAndValidateJSON(response);
      
      // Validiere die Features auch beim Retry
      const availableColumns = filterColumns(analysis.columns, selectedFeatures, excludedFeatures);
      recommendations.features = recommendations.features.filter(f => f !== recommendations.targetVariable);
      recommendations.features = recommendations.features.filter(f => availableColumns.includes(f));
    ...
    // Fallback verwenden
    console.log('Use fallback recommendations');
...
function filterDataOverviewForFeatures(dataOverview, selectedFeatures, excludedFeatures) {
  ...
  return `Filtered data overview:
  Available features: ${selectedFeatures ? selectedFeatures.join(', ') : 'All except: ' + excludedFeatures.join(', ')}
  ${dataOverview}`;
}
