...
export class DataAnalyzerWorker extends BaseWorker {
  ...
  async execute(pipelineState) {
    ...
    try {
      let dataAnalysis = await pythonClient.analyzeData(project.csvFilePath);
      ...
      this.log('success', 'Data analysis completed successfully');
      return enhancedAnalysis;

    } catch (error) {
      this.log('error', 'Data analysis failed', error.message);
      throw error;
    }
  }
  ...
  async performDataAnalysis(project) {
    const prompt = formatPrompt(DATA_ANALYSIS_PROMPT, {
      ...
      algorithm: project.algorithm || 'Not specified'
    });
